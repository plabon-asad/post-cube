...
<div class="ocdi  wrap  about-wrap">
	...
	<?php ob_start(); ?>
		<h1 class="ocdi__title  dashicons-before  dashicons-upload"><?php esc_html_e( 'Vast Demo Import', 'pt-ocdi' ); ?></h1>
	<?php
	...
	<?php if ( empty( $predefined_themes ) ) : ?>
		...
		<p class="ocdi__button-container">
			<button class="ocdi__button  button  button-hero  button-primary  js-ocdi-import-data"><?php esc_html_e( 'Vast Import Demo', 'pt-ocdi' ); ?></button>
		</p>
	...
	<?php else : ?>
		...
		?>
			<?php if ( ! empty( $categories ) ) : ?>
				<div class="row ocdi__gl-header  js-ocdi-gl-header">
					<nav class="col-lg-8 col-md-8 col-sm-8 col-xs-12 ocdi__gl-navigation">
						<ul>
					...
					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 ocdi__gl-search">
						<div class="ocdi__gl-cari pull-right">
							<input type="search" class="ocdi__gl-search-input  js-ocdi-gl-search" name="ocdi-gl-search" value="" placeholder="<?php esc_html_e( 'Search demo...', 'pt-ocdi' ); ?>"><i class="fa fa-search"></i>
						</div>
					</div>
				</div>
			<?php endif; ?>
			<div class="ocdi__gl-item-container  wp-clearfix  js-ocdi-gl-item-container">
				<?php foreach ( $predefined_themes as $index => $import_file ) : ?>
					...
					<div class="ocdi__gl-item js-ocdi-gl-item" data-categories="<?php echo esc_attr( Helpers::get_demo_import_item_categories( $import_file ) ); ?>" data-name="<?php echo esc_attr( strtolower( $import_file['import_file_name'] ) ); ?>">
						...
						<div class="row ocdi__gl-item-footer<?php echo ! empty( $import_file['preview_url'] ) ? '  ocdi__gl-item-footer--with-preview' : ''; ?>">
							<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 title"><h4 class="ocdi__gl-item-title" title="<?php echo esc_attr( $import_file['import_file_name'] ); ?>"><?php echo esc_html( $import_file['import_file_name'] ); ?></h4></div>
							<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 previewimportbutton">
								<?php if ( ! empty( $import_file['preview_url'] ) ) : ?>
									<a class="ocdi__gl-item-button prev button" href="<?php echo esc_url( $import_file['preview_url'] ); ?>" target="_blank"><?php esc_html_e( 'Preview', 'pt-ocdi' ); ?></a>
								<?php endif; ?>
								<button class="ocdi__gl-item-button  button import button-primary  js-ocdi-gl-import-data" value="<?php echo esc_attr( $index ); ?>"><?php esc_html_e( 'Import', 'pt-ocdi' ); ?></button>
							</div>
						</div>
					</div>
				<?php endforeach; ?>
